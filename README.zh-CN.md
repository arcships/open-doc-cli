...
**作为 Claude Code plugin（推荐）**——安装 `opendoc` plugin 后直接使用（例如让 Claude 搜索你的笔记）。引擎二进制不随仓库提交；skill 首次使用时会发现二进制缺失，在征得你同意后从 GitHub releases 下载对应平台的构建（`opendoc-<os>-<arch>`），并做 sha256 校验。

**或从源码构建：**

```bash
go build ./cmd/opendoc      # 构建 opendoc 二进制
./scripts/build-skill.sh    # 把 skill/ 打包为 Claude Code plugin
```
...
- [skill/SKILL.md](skill/SKILL.md) — plugin 内附带的 Agent Skill 引导。
